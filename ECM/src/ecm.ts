...
export function extractiveSummarize(segments: SegmentRecord[]): string {
  ...
  for (const seg of segments) {
    for (const sent of splitSentences(seg.content)) {
      allSentences.push({ text: sent, position: allSentences.length, segImportance: seg.importance });
    }
  }

  if (allSentences.length === 0) return segments.map(s => s.content).join(" ").slice(0, budget);

  const maxLen = Math.max(...allSentences.map(s => s.text.length), 1);
  const scored = allSentences.map(s => ({
    ...s,
    score: (1 / (1 + s.position * 0.1)) * 0.4 + (s.text.length / maxLen) * 0.3 + s.segImportance * 0.3,
  }));
...
export async function storeSegment(input: StoreSegmentInput): Promise<ToolResponse<SegmentRecord>> {
  try {
    ...
    const code = msg.startsWith("'") || msg.startsWith("Input") || msg.includes("must be")
      ? ErrorCode.INVALID_INPUT : ErrorCode.EXECUTION_FAILED;
    return createErrorResponse(code, msg) as ToolResponse<SegmentRecord>;
  }
}

export async function retrieveContext(input: RetrieveContextInput): Promise<ToolResponse<RetrieveResult>> {
  try {
    ...
    for (const { seg, score } of scored) {
      if (totalTokens + seg.token_count > maxTokens) { truncated = true; continue; }
...
export async function listSegments(input: ListSegmentsInput): Promise<ToolResponse<ListSegmentsResult>> {
  try {
    const validated = validateListSegments(input);
    const segments = store.listSegments(validated.sessionId, validated.limit ?? 20, validated.offset ?? 0);
...
export async function deleteSegment(input: DeleteSegmentInput): Promise<ToolResponse<DeleteSegmentResult>> {
  try {
    const validated = validateDeleteSegment(input);
    const existing = store.getSegmentById(validated.segmentId);
    if (!existing) {
      return createErrorResponse(ErrorCode.NOT_FOUND, `Segment not found: ${validated.segmentId}`) as ToolResponse<DeleteSegmentResult>;
    }
    const result = store.deleteSegment(validated.segmentId);
    return createSuccessResponse(result);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    return createErrorResponse(ErrorCode.EXECUTION_FAILED, msg) as ToolResponse<DeleteSegmentResult>;
  }
}

export async function clearSession(input: ClearSessionInput): Promise<ToolResponse<ClearSessionResult>> {
  try {
...
export async function summarizeSession(input: SummarizeSessionInput): Promise<ToolResponse<SummarizeResult>> {
  try {
    const validated = validateSummarizeSession(input);
    const toSummarize = store.getOldestNonSummarySegments(validated.sessionId, validated.keepNewest ?? 10);
