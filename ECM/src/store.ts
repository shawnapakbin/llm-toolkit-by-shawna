...
export class ECMStore {
  ...
  insertSegment(input: SegmentInsertInput): SegmentRecord {
    ...
    const stmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(id, input.sessionId, input.type, input.content, input.embeddingJson, input.tokenCount, input.metadataJson, input.importance);
    return this.getSegmentById(id) as SegmentRecord;
  }

  getSegmentById(id: string): SegmentRecord | undefined {
    return this.db.prepare("SELECT * FROM ecm_segments WHERE id = ?").get(id) as SegmentRecord | undefined;
  }

  getSegmentsBySession(sessionId: string): SegmentRecord[] {
    return this.db.prepare("SELECT * FROM ecm_segments WHERE session_id = ? ORDER BY created_at ASC").all(sessionId) as SegmentRecord[];
  }
  ...
  countSegments(sessionId: string): number {
    const row = this.db.prepare("SELECT COUNT(*) as cnt FROM ecm_segments WHERE session_id = ?").get(sessionId) as { cnt: number };
    return row.cnt;
  }
  ...
  deleteSegmentsByIds(ids: string[]): { deletedCount: number } {
    ...
    const result = this.db.prepare(`DELETE FROM ecm_segments WHERE id IN (${placeholders})`).run(...ids);
