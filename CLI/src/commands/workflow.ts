...
export function registerWorkflowCommands(program: Command): void {
  const workflow = program.command("workflow").description("Execute and manage AgentRunner workflows");
  ...
  workflow
    ...
    .action(async (file: string, opts: { session?: string; autoApprove?: boolean; timeout?: number }) => {
      // Resolve file path
      ...
      try {
        ...
        console.error(`Error: Failed to parse workflow JSON — ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      }
      ...
      try {
        ...
        console.log();
        console.log(`Workflow ${result.success ? "completed successfully" : "failed"} in ${result.durationMs}ms`);
