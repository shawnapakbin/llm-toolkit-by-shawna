...
app.post("/tools/git_status", async (req, res) => {
    ...
    try {
        ...
        if (!repoCheck.valid) {
            return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.INVALID_INPUT, repoCheck.error, Date.now() - start, traceId));
        }
...
app.post("/tools/git_branch", async (req, res) => {
    ...
    try {
        ...
        if (input.name) {
            ...
            if (!nameCheck.safe) {
                return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.POLICY_BLOCKED, nameCheck.reason, Date.now() - start, traceId));
            }
        }
        ...
        if (input.action === "delete" && input.force && input.name) {
            ...
            if (!forceCheck.allowed) {
                return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.POLICY_BLOCKED, forceCheck.reason, Date.now() - start, traceId));
            }
        }
...
app.post("/tools/git_commit", async (req, res) => {
    ...
    try {
        ...
        if (!messageCheck.valid) {
            return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.POLICY_BLOCKED, messageCheck.error, Date.now() - start, traceId));
        }
...
app.post("/tools/git_clone", async (req, res) => {
    ...
    try {
        ...
        if (!urlCheck.safe) {
            return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.POLICY_BLOCKED, urlCheck.reason, Date.now() - start, traceId));
        }
    ...
    try {
        ...
        if (input.mode === "hard") {
            ...
            if (!resetCheck.allowed) {
                return res.json((0, types_1.createErrorResponse)(types_1.ErrorCode.POLICY_BLOCKED, "Hard reset not allowed", Date.now() - start, traceId));
            }
        }
