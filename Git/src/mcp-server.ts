...
server.registerTool(
	...
	async ({ short }: any): Promise<CallToolResult> => {
...
server.registerTool(
	...
	async ({ target, staged, file }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	...
	async ({ maxCount, branch, file, oneline }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
	...
	"git_branch",
	{
		...
		inputSchema: {
			...
			force: z
				...
				.optional()
				.describe(
					"Force delete unmerged branch (default: false, blocked for protected branches)",
				),
		} as any,
	},
	async ({ action, name, force }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	...
	{
		description:
			"Switch to a different branch or commit. Can create a new branch while switching.",
		inputSchema: {
			target: z
				.string()
	...
	async ({ target, createBranch }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	"git_commit",
	{
		...
		inputSchema: {
			message: z
				...
				.min(3)
				.describe(
					"Commit message (min 3 chars, first line ≤100 chars recommended)",
				),
			all: z
				.boolean()
				.optional()
	...
	async ({ message, all, amend }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	"git_push",
	{
		...
		inputSchema: {
			...
			force: z
				...
				.optional()
				.describe(
					"Force push (default: false, BLOCKED for protected branches)",
				),
			setUpstream: z
				.boolean()
				.optional()
	...
	async ({ remote, branch, force, setUpstream }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	...
	async ({ remote, branch, rebase }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	"git_clone",
	{
		...
		inputSchema: {
			url: z
				.string()
				.describe(
					"Repository URL (https:// or git@ only, http:// and file:// blocked)",
				),
			directory: z
				.string()
				.optional()
	...
	async ({ url, directory, branch, depth }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	...
	async ({ action, message, index }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
...
server.registerTool(
	...
	async ({ mode, target }: any): Promise<CallToolResult> => {
		const repoPath = getGitWorkspaceRoot();
		try {
