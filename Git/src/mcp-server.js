...
server.registerTool("git_status", {
    description: "View Git repository status and modified files. Shows staged, unstaged, and untracked files.",
    inputSchema: {
        short: zod_1.z
            .boolean()
            .optional()
...
server.registerTool("git_diff", {
    description: "View Git diff showing changes between commits, branches, or working directory. Shows line-by-line changes.",
    inputSchema: {
        target: zod_1.z
            .string()
            .optional()
        ...
        staged: zod_1.z
            .boolean()
            .optional()
            .describe("Show staged changes only (git diff --staged)"),
        file: zod_1.z
            .string()
            .optional()
            .describe("Limit diff to specific file path"),
    },
...
server.registerTool("git_log", {
    description: "View Git commit history with filtering options. Shows commit hashes, messages, authors, and dates.",
    inputSchema: {
        maxCount: zod_1.z
            .number()
            .int()
        ...
        file: zod_1.z
            .string()
            .optional()
            .describe("Show log for specific file path only"),
        oneline: zod_1.z
            .boolean()
            .optional()
...
server.registerTool("git_branch", {
    description: "Manage Git branches: list all branches, create new branches, or delete branches. Protected branches (main/master/production) cannot be force-deleted.",
    inputSchema: {
        action: zod_1.z
            .enum(["list", "create", "delete"])
            .describe("Action to perform: 'list' (show all branches), 'create' (new branch), 'delete' (remove branch)"),
        name: zod_1.z
            .string()
            .optional()
...
server.registerTool("git_commit", {
    description: "Create a Git commit with validation. Commit messages must be at least 3 characters and avoid placeholders (wip, todo, fixme). First line should be ≤100 chars.",
    inputSchema: {
        message: zod_1.z
            .string()
            .min(3)
        ...
        all: zod_1.z
            .boolean()
            .optional()
            .describe("Stage all changes before commit (git commit -a)"),
        amend: zod_1.z
            .boolean()
            .optional()
...
server.registerTool("git_push", {
    description: "Push commits to remote repository. Force push is BLOCKED for protected branches (main/master/production). Set upstream tracking for new branches.",
    inputSchema: {
        remote: zod_1.z
            .string()
            .optional()
...
server.registerTool("git_pull", {
    description: "Pull changes from remote repository. Can use rebase instead of merge to maintain linear history.",
    inputSchema: {
        remote: zod_1.z
            .string()
            .optional()
        ...
        rebase: zod_1.z
            .boolean()
            .optional()
            .describe("Use rebase instead of merge (default: false)"),
    },
...
server.registerTool("git_clone", {
    description: "Clone a Git repository. Only HTTPS and SSH URLs are allowed (http:// and file:// are BLOCKED for security). Supports shallow clones for faster downloads.",
    inputSchema: {
        url: zod_1.z
            .string()
...
server.registerTool("git_stash", {
    description: "Manage Git stash: save uncommitted changes, restore stashed changes, list all stashes, or drop specific stashes. Useful for temporarily saving work.",
    inputSchema: {
        action: zod_1.z
            .enum(["save", "pop", "list", "drop"])
            .describe("Action: 'save' (stash changes), 'pop' (apply & remove stash), 'list' (show stashes), 'drop' (delete stash)"),
        message: zod_1.z
            .string()
            .optional()
            .describe("Stash message for 'save' action"),
        index: zod_1.z
            .number()
            .int()
            .nonnegative()
...
server.registerTool("git_reset", {
    description: "Reset repository to a previous state. SOFT (keep changes staged), MIXED (keep changes unstaged), HARD (discard all changes). HARD mode is DESTRUCTIVE and requires explicit confirmation.",
    inputSchema: {
        mode: zod_1.z
            .enum(["soft", "mixed", "hard"])
            .describe("Reset mode: 'soft' (keep staged), 'mixed' (keep unstaged), 'hard' (DISCARD ALL)"),
        target: zod_1.z
            .string()
            .optional()
            .describe("Commit/branch to reset to (default: 'HEAD')"),
    },
}, async ({ mode, target }) => {
    const repoPath = (0, policy_1.getGitWorkspaceRoot)();
    try {
