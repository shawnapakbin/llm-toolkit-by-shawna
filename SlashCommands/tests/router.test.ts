...
jest.mock("better-sqlite3", () => {
  const mockDb = {
    prepare: jest.fn().mockReturnValue({
      get: jest.fn().mockReturnValue({ total: 5, successes: 4, avg_ms: 120, min_ms: 80, max_ms: 200 }),
...
import { post, get } from "../src/dispatch";
...
describe("route — calculator", () => {
  it("posts expression to calculator endpoint", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.calculator}/tools/calculate_engineering`,
      { expression: "2 + 2" },
    );
  });
...
describe("route — webbrowser", () => {
  it("posts url and options to webbrowser endpoint", async () => {
    ...
    await route(desc);
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.webbrowser}/tools/browse_web`,
      { url: "https://example.com", outputFormat: "markdown", screenshot: false },
    );
  });
  ...
  it("includes waitForSelector when provided", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.webbrowser}/tools/browse_web`,
      { url: "https://example.com", outputFormat: "text", screenshot: true, waitForSelector: "#main" },
    );
  });
...
describe("route — clock", () => {
  it("posts to clock endpoint with no body when no timezone", async () => {
    ...
    await route(desc);
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.clock}/tools/get_current_datetime`,
      {},
    );
  });
...
describe("route — terminal", () => {
  it("posts command to terminal endpoint", async () => {
    ...
    await route(desc);
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.terminal}/tools/run_terminal_command`,
      { command: "ls -la" },
    );
  });
...
describe("route — skills", () => {
  ...
  it("posts execute_skill with params spread", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.skills}/tools/skills`,
      { action: "execute_skill", name: "my-skill", params: { key: "val" } },
    );
  });
...
describe("route — rag", () => {
  it("posts query action to rag endpoint", async () => {
    ...
    await route(desc);
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.rag}/tools/rag`,
      { action: "query", query: "what is AI", topK: 5 },
    );
  });
  ...
  it("posts ingest action with source", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.rag}/tools/rag`,
      { action: "ingest", content: "some text", source: "docs" },
    );
  });
  ...
  it("posts delete_source action", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.rag}/tools/rag`,
      { action: "delete_source", sourceId: "src-1" },
    );
  });
});
...
describe("route — askuser", () => {
  it("posts create_interview with prompt to askuser endpoint", async () => {
    ...
    await route(desc);
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.askuser}/tools/ask_user`,
  ...
  it("includes title when provided", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.askuser}/tools/ask_user`,
      { action: "create_interview", prompt: "Hello", title: "Greeting" },
    );
  });
  ...
  it("includes expiresInSeconds when provided", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      `${ENDPOINTS.askuser}/tools/ask_user`,
      { action: "create_interview", prompt: "Confirm?", expiresInSeconds: 60 },
    );
  });
...
describe("route — memory_stats", () => {
  ...
  it("returns success:false when DB is unavailable", async () => {
    ...
    Database.mockImplementationOnce(() => { throw new Error("no such file"); });
    const desc: DispatchDescriptor = { tool: "memory_stats" };
    const result = await route(desc) as Record<string, unknown>;
    expect(result.success).toBe(false);
