...
export type DispatchDescriptor =
  ...
  | { tool: "webbrowser"; url: string; outputFormat: string; screenshot: boolean; waitForSelector?: string }
  ...
  switch (cmd.toLowerCase()) {
    ...
    case "ecm": {
      ...
      switch ((sub ?? "").toLowerCase()) {
        ...
        case "retrieve":
          return {
            tool: "ecm", action: "retrieve_context",
            params: {
              sessionId,
        ...
        case "delete":
          return {
            tool: "ecm", action: "delete_segment",
            params: { sessionId, segmentId: positional[0] ?? flag(flags, "id") ?? "" },
          };
        case "summarize":
          return {
            tool: "ecm", action: "summarize_session",
            params: {
              sessionId,
              keepNewest: flag(flags, "keep-newest") ? Number(flag(flags, "keep-newest")) : undefined,
            },
          };
        case "clear":
    ...
    case "calc":
    case "calculate": {
      const expression = [sub, ...rest.filter(t => !t.startsWith("--"))].filter(Boolean).join(" ");
    ...
    case "skills": {
      ...
      switch (action) {
        ...
        case "execute": {
          ...
          if (paramsFlag) {
            try { params = JSON.parse(paramsFlag); } catch { /* ignore */ }
          }
          return { tool: "skills", action: "execute_skill", params: { name: positional[0], params } };
        }
        case "delete":
    ...
    case "memory": {
      switch ((sub ?? "").toLowerCase()) {
        ...
        case "history":
          return { tool: "memory_history", limit: flag(flags, "limit") ? Number(flag(flags, "limit")) : 20 };
        case "patterns":
