...
export async function executeSkill(
  ...
  try {
    ...
    if (!skill) {
      return createErrorResponse(ErrorCode.NOT_FOUND, `Skill not found: ${validated.name}`) as ToolResponse<ExecuteSkillResult>;
    }
    ...
    if (message.startsWith("'") || message.startsWith("Input")) {
      return createErrorResponse(ErrorCode.INVALID_INPUT, message) as ToolResponse<ExecuteSkillResult>;
    }
    return createErrorResponse(ErrorCode.EXECUTION_FAILED, message) as ToolResponse<ExecuteSkillResult>;
  }
}
...
export async function getSkill(input: GetSkillInput): Promise<ToolResponse<SkillRecord>> {
  try {
    ...
    if (!skill) {
      ...
      return createErrorResponse(ErrorCode.NOT_FOUND, `Skill not found: ${identifier}`) as ToolResponse<SkillRecord>;
    }
...
export async function listSkills(input: ListSkillsInput): Promise<ToolResponse<SkillListResult>> {
  try {
    ...
    return createErrorResponse(ErrorCode.EXECUTION_FAILED, message) as ToolResponse<SkillListResult>;
  }
}

export async function deleteSkill(input: DeleteSkillInput): Promise<ToolResponse<DeleteSkillResult>> {
  try {
    ...
    if (!skill) {
      const identifier = validated.id ?? validated.name ?? "unknown";
      return createErrorResponse(ErrorCode.NOT_FOUND, `Skill not found: ${identifier}`) as ToolResponse<DeleteSkillResult>;
    }
    ...
    const message = err instanceof Error ? err.message : String(err);
    if (message.startsWith("At least") || message.startsWith("Input")) {
      return createErrorResponse(ErrorCode.INVALID_INPUT, message) as ToolResponse<DeleteSkillResult>;
    }
    return createErrorResponse(ErrorCode.EXECUTION_FAILED, message) as ToolResponse<DeleteSkillResult>;
  }
}
