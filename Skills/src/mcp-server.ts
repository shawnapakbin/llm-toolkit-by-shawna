...
const skillsInputShape = {
  ...
  // define_skill fields
  name: z.string().optional().describe("(define_skill/execute_skill/get_skill/delete_skill) Kebab-case skill name."),
      ...
      switch (action) {
        case "define_skill":
          result = await defineSkill({
            ...
            steps: steps as SkillsInput["steps"] & Array<{ type: "prompt" | "tool_call"; template: string; tool: string; args: Record<string, string> }>,
          });
          break;
        ...
        default: {
          ...
          result = { success: false, error: { code: "INVALID_INPUT", message: `Unknown action: ${_exhaustive}` } };
        }
      }
