...
function validateTargetUrl(urlValue: string): { ok: true } | { ok: false; message: string; errorCode: string } {
  try {
    const parsed = new URL(urlValue);
    if (!["http:", "https:"].includes(parsed.protocol)) {
      return { ok: false, message: "Only http and https URLs are allowed.", errorCode: "INVALID_INPUT" };
    }
