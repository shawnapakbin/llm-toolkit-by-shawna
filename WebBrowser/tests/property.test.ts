    ...
    fc.assert(
      fc.property(
        ...
        (a, b, c, d) => {
          ...
          if (host.startsWith("10.") || host.startsWith("192.168.") || host.startsWith("172.")) return;
...
describe("Property: content-type allowlist", () => {
  test("allowed content types always pass", () => {
    const allowed = ["text/html", "text/plain", "application/xhtml+xml", "application/xml", "application/json"];
    fc.assert(
  ...
  test("binary/image content types are always blocked", () => {
    const blocked = ["image/png", "image/jpeg", "image/gif", "application/octet-stream", "video/mp4", "audio/mpeg"];
    fc.assert(
