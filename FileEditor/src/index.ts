...
app.post("/tools/read_file", async (req, res) => {
  ...
  try {
    ...
    if (!pathCheck.valid) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, pathCheck.error!, Date.now() - start, traceId));
    }

    const blockedCheck = isBlockedPath(input.path);
    if (blockedCheck.blocked) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, blockedCheck.reason!, Date.now() - start, traceId));
    }
...
app.post("/tools/write_file", async (req, res) => {
  ...
  try {
    ...
    if (!extCheck.allowed) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, extCheck.reason!, Date.now() - start, traceId));
    }

    const contentCheck = validateContentSafety(input.content);
    if (!contentCheck.safe) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, contentCheck.reason!, Date.now() - start, traceId));
    }
...
app.post("/tools/search_files", async (req, res) => {
  ...
  try {
    ...
    res.json(createSuccessResponse({ results: result, count: result.length }, Date.now() - start, traceId));
  } catch (error: any) {
    if (error.name === "ZodError") {
      return res.json(createErrorResponse(ErrorCode.INVALID_INPUT, error.errors[0].message, Date.now() - start, traceId));
    }
    return res.json(createErrorResponse(ErrorCode.EXECUTION_FAILED, error.message, Date.now() - start, traceId));
  }
});
...
app.post("/tools/list_directory", async (req, res) => {
  ...
  try {
    ...
    res.json(createSuccessResponse({ files: result, count: result.length }, Date.now() - start, traceId));
  } catch (error: any) {
    if (error.name === "ZodError") {
      return res.json(createErrorResponse(ErrorCode.INVALID_INPUT, error.errors[0].message, Date.now() - start, traceId));
    }
...
app.post("/tools/delete_file", async (req, res) => {
  ...
  try {
    ...
    if (!deleteCheck.allowed) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, deleteCheck.reason!, Date.now() - start, traceId));
    }
...
app.post("/tools/move_file", async (req, res) => {
  ...
  try {
    ...
    if (!sourceCheck.valid) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, sourceCheck.error!, Date.now() - start, traceId));
    }

    const destCheck = validatePath(input.destination, WORKSPACE_ROOT);
    if (!destCheck.valid) {
      return res.json(createErrorResponse(ErrorCode.POLICY_BLOCKED, destCheck.error!, Date.now() - start, traceId));
    }
